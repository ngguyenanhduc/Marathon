...
@WebServlet(
        name = "OrganizerRegistrationApproveServlet",
        urlPatterns = {"/organizer/registration/approve"}
)
public class OrganizerRegistrationApproveServlet
        extends HttpServlet {
    ...
    protected void doPost(
        ...
        boolean approved =
                registrationDAO.approveRegistration(
                        registrationId,
                        organizer.getUserId()
                );
        ...
        if (approved) {
            session.setAttribute(
                    "successMessage",
                    "Đã duyệt đăng ký và tự động gán số BIB."
            );
        } else {
            session.setAttribute(
                    "errorMessage",
                    "Không thể duyệt đăng ký. "
                    + "Đăng ký có thể đã được xử lý."
            );
        }
