 ...
 *
 * Purpose: bao ve cac URL chi danh cho Runner
 */
public class RunnerAuthorizationFilter implements Filter {

    @Override
    public void init(FilterConfig filterConfig) throws ServletException {
    }

    //kiem tra session, role va trang thai truoc khi vao URL /runner/*
    @Override
    public void doFilter(ServletRequest servletRequest,
        ...
        if (!user.isRunner()) {
            response.sendError(HttpServletResponse.SC_FORBIDDEN,
                    "Ban khong co quyen truy cap chuc nang Runner.");
            return;
        }

        //huy session cua tai khoan Runner khong con ACTIVE
